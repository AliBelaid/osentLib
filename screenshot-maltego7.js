...
const CDP_PORT = 9240;
const APP_PORT = 4100;  // AU Sentinel is on port 4100!
const API_PORT = 9099;
...
function getPageWsUrl() {
  return new Promise((resolve, reject) => {
    ...
    function tryGet() {
      ...
      req.on('error', () => { if (retries < 20) { retries++; setTimeout(tryGet, 500); } else reject(new Error('Conn fail')); });
    }
    tryGet();
  });
}
...
async function main() {
  ...
  const tmpDir = 'C:\\temp\\chrome-m7-' + Date.now();
  ...
  // Load app to get same-origin context
  console.log('3. Load app origin...');
  await cdp.send('Page.navigate', { url: `http://localhost:${APP_PORT}` });
  await sleep(5000);

  let r = await cdp.send('Runtime.evaluate', { expression: 'document.title' });
  console.log('   Title:', r.result?.result?.value);

  // Login via fetch API from browser
  console.log('4. Login via fetch...');
  r = await cdp.send('Runtime.evaluate', {
    expression: `
      fetch('http://localhost:${API_PORT}/api/auth/login', {
        method: 'POST',
  ...
  console.log('   Login:', r.result?.result?.value);

  // Navigate to /maltego (full page reload picks up localStorage)
  console.log('5. Navigate to /maltego...');
  await cdp.send('Page.navigate', { url: `http://localhost:${APP_PORT}/maltego` });

  // Wait for router-outlet (Angular loaded)
  console.log('   Waiting for Angular...');
  await waitFor(cdp, `!!document.querySelector('router-outlet')`, 20000);
  await sleep(5000);

  r = await cdp.send('Runtime.evaluate', { expression: 'window.location.href + " | title: " + document.title' });
    ...
    expression: `
      (function(){
        ...
        for (var j = 0; j < btns.length; j++) {
          var t = (btns[j].textContent || '').trim().substring(0, 30);
          info += '[' + t + '] ';
          if (t.toLowerCase().indexOf('transform') >= 0) {
            btns[j].click();
            info += 'CLICKED! ';
            break;
          }
