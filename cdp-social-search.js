...
async function run() {
  ...
  if (!page) { console.log('No page tab found'); return; }
  ...
  ws.on('open', async () => {
    try {
      ...
      fs.writeFileSync('C:\\osentLib\\screen\\social-search-empty.png', Buffer.from(screenshot.data, 'base64'));
      console.log('Screenshot saved: screen/social-search-empty.png');

      // Now type in the search box and search
      // First, focus and click the search input
      ...
      await send('Runtime.evaluate', {
        expression: `
          ...
          if (input) {
            ...
            nativeInputValueSetter.call(input, 'cybersecurity');
            input.dispatchEvent(new Event('input', { bubbles: true }));
            input.dispatchEvent(new Event('change', { bubbles: true }));
            'input found and value set';
          } else {
            'no input found';
          }
        `
      ...
      fs.writeFileSync('C:\\osentLib\\screen\\social-search-results.png', Buffer.from(screenshot2.data, 'base64'));
      console.log('Screenshot saved: screen/social-search-results.png');
      ...
      console.log('Page state:', pageContent.result?.value || 'unknown');
