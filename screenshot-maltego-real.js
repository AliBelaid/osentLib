...
async function main() {
  ...
  const userDataDir = `${process.env.TEMP}\\chrome-cdp-mr6-${Date.now()}`;
  ...
  // Login
  console.log('1. Login...');
  await cdpSend(ws, cmdId++, 'Page.navigate', { url: `http://localhost:${APP_PORT}/login` });
  await new Promise(r => setTimeout(r, 5000));
  await cdpSend(ws, cmdId++, 'Runtime.evaluate', {
    expression: `
      (async () => {
        const resp = await fetch('/api/auth/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username: 'admin', password: 'Admin123!' }) });
  ...
  // Navigate to Maltego
  console.log('2. Navigate to /maltego...');
  await cdpSend(ws, cmdId++, 'Page.navigate', { url: `http://localhost:${APP_PORT}/maltego` });
  await new Promise(r => setTimeout(r, 7000));

  // Directly call the API from the browser and feed results into the component
  console.log('3. Calling transform API directly from browser...');
  const apiCallScript = `
    (async () => {
      const token = localStorage.getItem('ausentinel_token');
      try {
        const resp = await fetch('/api/osint/maltego/transform', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token },
          body: JSON.stringify({ entityType: 'Domain', entityValue: 'google.com', transformType: 'default' })
        });
        if (!resp.ok) return 'api_error:' + resp.status + ' ' + await resp.text();
        const data = await resp.json();

        // Get the component and feed the data
        const el = document.querySelector('app-maltego-graph') || document.querySelector('router-outlet + *');
        const comp = window.ng?.getComponent(el);
        if (comp) {
          comp.entityValue = 'google.com';
          comp.entityType = 'Domain';
          comp.mergeResults(data, null);
          comp.loading = false;
          comp.cdr.markForCheck();
          comp.cdr.detectChanges();
          return 'entities:' + data.entities?.length + ' links:' + data.links?.length + ' nodes_after:' + comp.nodes?.length;
        }
        return 'no_component. entities:' + data.entities?.length;
      } catch(e) {
        return 'error:' + e.message;
      }
    })()
  `;
  const apiResult = await cdpSend(ws, cmdId++, 'Runtime.evaluate', {
    expression: apiCallScript, awaitPromise: true, returnByValue: true
  });
  console.log('   Result:', apiResult.result?.value);

  // Wait for graph to render/stabilize
  console.log('4. Waiting 5s for graph render...');
  await new Promise(r => setTimeout(r, 5000));

  // Check graph state
  const gs = await cdpSend(ws, cmdId++, 'Runtime.evaluate', {
    expression: `JSON.stringify({ svgs: document.querySelectorAll('svg').length, circles: document.querySelectorAll('circle').length, lines: document.querySelectorAll('line').length, texts: document.querySelectorAll('svg text').length })`,
    returnByValue: true
  });
  console.log('   Graph DOM:', gs.result?.value);
  ...
  fs.writeFileSync('C:\\osentLib\\screenshot-maltego-real.png', Buffer.from(shot.data, 'base64'));
  console.log('   Saved!');
