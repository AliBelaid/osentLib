  ...
  ws.on('open', async () => {
    try {
      const result = await send('Runtime.evaluate', {
        expression: `
          (async () => {
            ...
            async function testGet(name, url) {
              try {
                ...
                try {
                  ...
                  else if (j.items) info = j.items.length + ' items';
                  else info = Object.keys(j).join(',').substring(0, 60);
            ...
            await testPost('POST /api/dns/extract-domains', '/api/dns/extract-domains', { text: 'Visit github.com and google.com' });
            ...
            await testPost('POST /api/external-search/search', '/api/external-search/search', { provider: 'GitHub', query: 'osint', filters: {} });
            await testPost('POST /api/external-search/search', '/api/external-search/search', { provider: 'Reddit', query: 'osint', filters: {} });
            await testPost('POST /api/external-search/search', '/api/external-search/search', { provider: 'HackerNews', query: 'osint', filters: {} });
            await testPost('POST /api/external-search/search', '/api/external-search/search', { provider: 'ThreatIntel', query: 'github.com', filters: {} });
            ...
            // === VOTE ===
            // Skip vote since it needs a valid article ID
      ...
      tests.forEach(t => {
        const s = t.status;
        let icon;
        if (s >= 200 && s < 300) { icon = '✅'; pass++; }
        else if (s >= 400 && s < 500) { icon = '⚠️'; warn++; }
        else { icon = '❌'; fail++; }
        console.log(`${icon} [${s}] ${t.name} → ${t.info} (${t.bytes}b)`);
      });
