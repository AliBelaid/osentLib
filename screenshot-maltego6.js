      ...
      const req = http.get(`http://127.0.0.1:${PORT}/json/list`, res => {
        ...
        res.on('end', () => {
          try {
            ...
            else reject(new Error('No page tabs. Got: ' + JSON.stringify(tabs.map(t => ({type: t.type, url: t.url})))));
...
class CDP {
  constructor(wsUrl) { this.wsUrl = wsUrl; this.msgId = 1; this.pending = new Map(); this.buf = Buffer.alloc(0); this.sock = null; }
  connect() {
    return new Promise((res, rej) => {
      const u = new URL(this.wsUrl), k = crypto.randomBytes(16).toString('base64');
      this.sock = net.createConnection({ host: u.hostname, port: parseInt(u.port) }, () => {
        this.sock.write(`GET ${u.pathname} HTTP/1.1\r\nHost: ${u.host}\r\nUpgrade: websocket\r\nConnection: Upgrade\r\nSec-WebSocket-Key: ${k}\r\nSec-WebSocket-Version: 13\r\n\r\n`);
      });
      let hd = false;
      this.sock.on('data', c => {
        if (!hd) { const s = c.toString(); if (s.includes('101')) { hd = true; const i = s.indexOf('\r\n\r\n'); if (i !== -1 && i + 4 < c.length) this.buf = Buffer.concat([this.buf, c.slice(i + 4)]); this._pf(); res(); } }
  ...
  _pf() {
    while (true) {
      ...
      try { const m = JSON.parse(p.toString()); if (m.id && this.pending.has(m.id)) { const c = this.pending.get(m.id); this.pending.delete(m.id); clearTimeout(c.t); c.r(m); } } catch {}
    }
  }
  _ws(d) {
  ...
  send(method, params = {}, tms = 60000) {
    return new Promise(r => { const id = this.msgId++; const t = setTimeout(() => { this.pending.delete(id); r({ error: 'timeout', method }); }, tms); this.pending.set(id, { r, t }); this._ws({ id, method, params }); });
  }
...
async function main() {
  ...
  const tmpDir = 'C:\\temp\\chrome-m6-' + Date.now();
  ...
  // Step 1: Load the app root on the correct origin
  console.log('3. Load app...');
  await cdp.send('Page.navigate', { url: 'http://localhost:4200' });
  await sleep(5000);

  // Step 2: Do login via fetch API in browser context
  console.log('4. Login via fetch...');
  let r = await cdp.send('Runtime.evaluate', {
    expression: `
      ...
      .then(data => {
        ...
        return 'LOGIN OK: ' + data.user.username + ' roles:' + JSON.stringify(data.user.roles);
      })
      .catch(e => 'LOGIN FAILED: ' + e.message)
    `,
    awaitPromise: true
  });
  console.log('   Result:', r.result?.result?.value);

  // Step 3: Reload the page to pick up the auth state, navigate to /maltego
  console.log('5. Navigate to /maltego...');
  await cdp.send('Page.navigate', { url: 'http://localhost:4200/maltego' });

  // Wait for Angular to bootstrap and route to Maltego
  ...
  console.log('   DOM:', JSON.stringify(r.result?.result?.value));

  // Interact with Maltego controls
