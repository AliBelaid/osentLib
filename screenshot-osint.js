...
const CDP_PORT = 9242;
const APP_PORT = 4100;
const OUTPUT = 'C:\\osentLib\\screenshot-osint.png';
...
async function main() {
  ...
  const tmpDir = 'C:\\temp\\chrome-osint-' + Date.now();
  ...
  console.log('5. Navigate /osint-tools...');
  await cdp.send('Page.navigate', { url: `http://localhost:${APP_PORT}/osint-tools` });
  ...
  r = await cdp.send('Runtime.evaluate', {
    expression: `JSON.stringify({i:document.querySelectorAll('input').length, b:document.querySelectorAll('button').length, tabs:document.querySelectorAll('.mat-mdc-tab, .mdc-tab').length, nav:!!document.querySelector('mat-sidenav')})`
  });
  console.log('   DOM:', r.result?.result?.value);

  // Trigger an email breach check to show results
  console.log('6. Trigger email breach check...');
  r = await cdp.send('Runtime.evaluate', {
    expression: `
      (function(){
        ...
        // Find the email input (first text input)
        for (var i = 0; i < inputs.length; i++) {
          if (inputs[i].type === 'text' || inputs[i].type === 'email' || !inputs[i].type) {
            setter.call(inputs[i], 'admin@example.com');
            inputs[i].dispatchEvent(new Event('input', {bubbles: true}));
            info += 'SET[' + i + '] ';
            break;
          }
        }
        // Find and click the check/search button
        var btns = document.querySelectorAll('button');
        info += btns.length + ' btns: ';
        for (var j = 0; j < btns.length; j++) {
          var t = (btns[j].textContent || '').toLowerCase().trim();
          info += '[' + t.substring(0,20) + '] ';
          if (t.indexOf('check') >= 0 || t.indexOf('breach') >= 0 || t.indexOf('search') >= 0 || t.indexOf('scan') >= 0) {
            btns[j].click();
            info += 'CLICKED! ';
            break;
          }
