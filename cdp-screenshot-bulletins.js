...
const OUTPUT = 'C:\\osentLib\\screen\\bulletins-page.png';
const TEMP_DIR = process.env.TEMP + '\\cdp-bulletins-' + Date.now();
const PORT = 9233;
...
async function main() {
  ...
  await ws.call('Runtime.evaluate', { expression: `
    localStorage.setItem('ausentinel_token', '${token}');
    localStorage.setItem('ausentinel_user', '${userJson}');
    localStorage.setItem('preferredLanguage', 'en');
    'done';
  ` });

  // Navigate to bulletins page
  console.log('Navigating to /bulletins...');
  await ws.call('Page.navigate', { url: 'http://localhost:8088/bulletins' });
